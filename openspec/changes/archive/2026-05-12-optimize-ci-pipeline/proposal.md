...
The `add-ci-pipeline` change introduced a working CI/CD pipeline, but a subsequent audit (informed by research into 10k+ star open-source projects) identified correctness and performance issues: Docker image builds have zero layer caching, four Node.js CI jobs duplicate identical setup boilerplate, `helm-validate` is unnecessarily serialized behind unrelated checks, the CI workflow has no `pnpm build` gate leaving compiled output unverified before merge, the deploy workflow redundantly runs on PRs once CI is the quality gate, app and Helm checks run even when unrelated files changed, and there are no branch protection rules enforcing CI as a merge requirement.
...
- Add a `changes` job using path filters so app jobs run only for app/tooling changes and Helm validation runs only for chart changes
- Add a final `status-gate` job that always runs and fails only when required upstream jobs fail or are cancelled, while accepting intentionally skipped path-filtered jobs
...
- `ci-pipeline`: The `build` job addition, path-aware job filtering, `status-gate`, and `helm-validate` DAG change constitute requirement-level behavior changes to the existing CI pipeline spec

## Impact

- `.github/workflows/ci.yml` — add `build` job, refactor 4 jobs to use composite action, add path filtering, add `status-gate`, remove direct `lint`/`type-check` dependency from `helm-validate`
