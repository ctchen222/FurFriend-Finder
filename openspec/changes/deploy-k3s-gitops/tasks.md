...
- [ ] 3.1 Configure GitHub Actions and GHCR settings needed for CI image publishing.
- [ ] 3.2 Decide whether the GHCR package is public or private; if private, configure image pull access for the cluster.
- [ ] 3.3 Install single-node k3s on the VPS and verify node readiness.
- [ ] 3.4 Configure firewall and DNS so only SSH, HTTP, and HTTPS are exposed.
- [ ] 3.5 Install cert-manager and verify a production ClusterIssuer can issue TLS certificates.
- [ ] 3.6 Install Argo CD and connect it to the repository deployment path.
- [ ] 3.7 Create and back up the SOPS age private key; provide only the public recipient for repo configuration.
- [ ] 3.8 Encrypt production Kubernetes Secrets locally with SOPS, commit only encrypted manifests, then locally decrypt/apply the Kubernetes Secret to the cluster before first sync.
- [ ] 3.9 Sync the production Argo CD application and verify database schema, app readiness, and `/health` over HTTPS.
- [ ] 3.10 Trigger and verify the first PostgreSQL backup, then rehearse restore into a safe target.
- [ ] 3.11 Configure or document an off-VPS backup copy path before treating backups as VPS-loss protection.

## 4. Handoff and Feedback Loop

- [ ] 4.1 Owner reports non-sensitive bootstrap values: domain, namespace, storageClass, ingress class if non-default, and Argo CD app status.
- [ ] 4.2 Adjust Helm values/templates based on owner-reported cluster behavior.
- [ ] 4.3 Owner reports non-sensitive rollout output: Pod readiness, certificate readiness, migration/schema Job status, `/health` status, and backup file name.
- [ ] 4.4 Resolve deployment issues through repo patches and repeat Argo CD sync until Healthy/Synced.
