...
export class OrganizationRepository {
    ...
    async account(actorId: string, creationLock = false) {
        const result = await this.db.query<{ emailVerified: boolean }>(
            `SELECT "emailVerified" FROM "user" WHERE id=$1 ${creationLock ? 'FOR UPDATE' : 'FOR SHARE'}`,
            [actorId],
        );
        return result.rows[0];
    }

    async creationCount(actorId: string): Promise<number> {
    ...
    async creation(actorId: string, requestId: string) {
        const result = await this.db.query<{
            ...
            'SELECT id,creation_hash FROM organizations WHERE created_by=$1 AND creation_key=$2',
            [actorId, requestId],
        );
        return result.rows[0];
    }
    ...
    async addOwner(id: string, actorId: string) {
        ...
        await this.db.query(
            `INSERT INTO organization_audit_events (organization_id,actor_id,action) VALUES ($1,$2,'CREATED')`,
            [id, actorId],
        );
    }
    ...
    async workspace(
        id: string,
        actorId: string,
    ): Promise<OrganizationWorkspace | undefined> {
        const result = await this.db.query<OrganizationWorkspace>(
    ...
    async list(
        ...
        const result = await this.db.query<OrganizationWorkspace>(
            ...
            [actorId, query.cursor ?? null, query.pageSize + 1],
        );
        return result.rows;
    }
}
