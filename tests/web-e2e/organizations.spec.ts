...
test('creates an organization, preserves retry identity, and shows private workspace status', async ({
    page,
}) => {
    ...
    await expect(
        page.getByRole('heading', { name: '尚未加入中途之家' }),
    ).toBeVisible();
    ...
    await expect(
        page.getByRole('heading', { name: '小橘中途之家', exact: true }),
    ).toBeVisible();
    await expect(page.getByText(/公開介紹與動物刊登會在/)).toHaveCount(0);
    await expect(page.getByRole('heading', { name: '組織狀態' })).toHaveCount(
        0,
    );
...
test('switching organizations never displays the previous private workspace and handles revoked access', async ({
    page,
}) => {
    ...
    await page.route(`**/api/v1/organizations/${otherId}/profile`, (route) =>
        route.fulfill({ status: 404, json: { message: '找不到此組織' } }),
    );
    ...
    await page
        .getByRole('link', { name: '另一間中途之家', exact: true })
        .click();
...
test('unauthenticated organization deep links retain a safe Google callback', async ({
    page,
}) => {
    await page.route('**/api/v1/me', (route) =>
...
test('compact workspace keeps actionable status without empty-field noise', async ({
    page,
}) => {
